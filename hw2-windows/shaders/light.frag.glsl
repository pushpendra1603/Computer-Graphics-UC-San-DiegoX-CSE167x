    ...
    if (enablelighting) {       
        ...
        for (int i = 0; i<numused; i++) {
            if (lightposn[i].w != 1.0) {
                ...
                finalcolor = finalcolor + ComputeLight(direction, lightcolor[i], normal, half1, diffuse, specular, shininess) ;
            }
            else {
                vec3 position = lightposn[i].xyz / lightposn[i].w;
                vec3 direction1 = normalize (position - mypos);
                vec3 half1 = normalize(direction1 + eyedirn);
                finalcolor = finalcolor + ComputeLight(direction1, lightcolor[i], normal, half1, diffuse, specular, shininess) ;
            }
        }
